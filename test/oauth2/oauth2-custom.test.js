...
describe('OAuth2', () => {
  describe('custom grant', () => {
    const baseConfig = Object.freeze({
      ...
      scopes: ['a', 'b'],
      accessTokenUri: new URL('/oauth2/custom-grant', document.baseURI).toString(),
    });
    ...
    describe('requesting the data', () => {
      ...
      it('throws when invalid grant', async () => {
        ...
        assert.typeOf(err, 'error', 'has the error');
        assert.typeOf(err.message, 'string', 'message is set');
        assert.equal(err.code, 'invalid_grant', 'code is set');
      });
    });
